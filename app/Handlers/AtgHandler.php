...
final class AtgHandler extends Handler implements HandlerInterface
{
    ...
    public function handle(): void
    {
        ...
        // event broadcast 
        AtgCalculateEvent::dispatch($data + [
    ...
    protected function getVolume($data, $atg, $height): array {
        ...
        $heightMM = floor($height / $this->conversi);
        // $heightCmRound = round(($height / $this->conversi) - $heightCm, 1, PHP_ROUND_HALF_UP);
        $volumeTable = VolumeTable::where('atg_id', $atg->id)->where('height', $heightMM)->first();
