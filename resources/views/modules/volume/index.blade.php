                ...
                <div class="btn-list">
                    <div class="dropdown d-none d-sm-inline">
                        ...
                        <div class="dropdown-menu dropdown-menu-demo">
                            ...
                            <a href="{{ route('atgsetting.table-volume.index', ['atg_id' => $atg->id]  + request()->all()) }}" class="dropdown-item">{{ $atg->name }}</a>
                            @endforeach
                        </div>
                    </div>
                    @if(Auth::user()->role == 'admin')
                    <a href="{{ route('atgsetting.table-volume.create') }}" class="btn btn-primary d-none d-sm-inline-block">
                ...
                <table class="table table-vcenter card-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-end">Different</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->atg->name }}</td>
                            <td class="text-end">{{ number_format($item->height) }}</td>
                            <td class="text-end">{{ number_format($item->volume, 2) }}</td>
                            <td class="text-end">{{ number_format($item->different, 2) }}</td>
