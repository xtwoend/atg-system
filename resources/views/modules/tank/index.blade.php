    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                ...
                <div class="btn-list">
                    <a href="{{ route('atgsetting.tank.create') }}" class="btn btn-primary d-none d-sm-inline-block">
                ...
                <table class="table table-vcenter card-table">
                    <thead>
                        <tr>
                            ...
                            <th>Location</th>
                            <th>Capacity</th>
                            <th>Factore Correction</th>
                            <th>Temperature Sounding</th>
                            <th>Added At</th>
                            @if(Auth::user()->role == 'admin')
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->location }}</td>
                            <td>{{ $item->capacity }}</td>
                            <td>{{ $item->factor_correction }}</td>
                            <td>{{ $item->temperature }}</td>
                            <td>{{ $item->created_at }}</td>
                            @if(Auth::user()->role == 'admin')
                            <td class="text-end">
                                <a href="{{ route('atgsetting.tank.edit', $item->id) }}">Edit</a> | <a href="#" onclick="event.preventDefault(); document.getElementById('del-{{$item->id}}').submit();"> Delete <a>
                                <form id="del-{{$item->id}}" action="{{ route('atgsetting.tank.destroy', $item->id) }}" method="POST" class="d-none">
                                    @method("DELETE")
                                    @csrf
                                </form>
                            </td>
